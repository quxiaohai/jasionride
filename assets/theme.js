...
class RecentlyViewed extends HTMLElement {
    ...
    getQueryString() {
        ...
        return Array.from(items.values(), (item) => `id:${item}`)
            .slice(0, this.hasAttribute('data-limit') ? parseInt(this.getAttribute('data-limit')) : 4)
            .join(' OR ');
    }
}
...
class MarqueeElement extends HTMLElement {
    ...
    init() {
        ...
        if (this.parallax) {
            ...
            Motion.scroll(Motion.animate(this, {transform: [`translateX(${translate}%)`, `translateX(0)`]}, {easing: 'linear'}), {
                target: this,
...
class ScrolledImages extends HTMLElement {
    ...
    setupParallax() {
        Array.from(this.querySelectorAll('.scrolled-images__row')).forEach((element, index) => {
            ...
            if (index % 2 === 0) {
                Motion.scroll(Motion.animate(element, {transform: [`translateX(${translate}%)`, 'translateX(0)']}, {easing: 'linear'}), {
                    target: this,
...
class ParallaxElement extends HTMLDivElement {
    ...
    motion() {
        this.animation = Motion.scroll(
            Motion.animate(this, {
                transform: [`translateY(${this.getAttribute('data-start')})`, `translateY(${this.getAttribute('data-stop')})`]
            }),
...
class SliderElement extends HTMLElement {
    ...
    get motionList() {
        return this.querySelector('motion-list');
    }
    ...
    previous() {
        this.scrollPosition = this.scrollLeft - this.itemOffset * (theme.config.rtl ? -1 : 1);
    ...
    update() {
        ...
        console.log(`Current scroll percentage: ${scrollPercentage.toFixed(2)}%`);

        // 更新 .slider-progress 的 ::after 伪元素的宽度
        const progressBar = this.nextElementSibling;
        if (progressBar && progressBar.classList.contains('slider-progress')) {
            let attrName = window.innerWidth > 768 ? 'og-progress-width' : 'og-progress-width-mb';
            let progressWidth = progressBar.getAttribute(attrName);
            progressWidth = parseFloat(progressWidth) || 0;
            let n = 100 - progressWidth;
            let n2 = (scrollPercentage * n) / 100;
            progressBar.style.setProperty('--progress-width', `${progressWidth + n2}%`);
        }
    }
    ...
    isVisible(element, offset = 0) {
        ...
        return element.offsetLeft - this.offsetLeft + element.clientWidth <= lastVisibleSlide && element.offsetLeft >= this.scrollLeft;
...
class ProductInfo extends HTMLElement {
    ...
    connectedCallback() {
        ...
        this.onVariantChangeUnsubscriber = theme.pubsub.subscribe(theme.pubsub.PUB_SUB_EVENTS.optionValueSelectionChange, this.handleOptionValueChange.bind(this));
...
class ProductForm extends HTMLFormElement {
    ...
    onSubmitHandler(event) {
        ...
        // Determine if the bundle exists
    ...
    toggleSubmitButton(disable = true, text, unavailable = false) {
        ...
        if (disable) {
            ...
            (submitButtonTextChild || submitButtonText).textContent = this.submitButton.hasAttribute('data-pre-order')
...
class TabsElement extends HTMLElement {
    constructor() {
        ...
        // 判断是否有mode="hover"
        const isHoverMode = this.hasAttribute('mode') && this.getAttribute('mode') === 'hover';
        console.log('isHoverMode:', isHoverMode); // Log mode check

        // 为按钮添加事件
        this.buttons.forEach((button, index) => {
            if (isHoverMode) {
                // 使用 hover 模式，改为 mouseover 事件
                button.addEventListener('mouseover', () => {
                    console.log('Hovered on tab:', index); // Log hover event
                });
            } else {
                // 默认点击事件
    ...
    attributeChangedCallback(name, oldValue, newValue) {
        console.log('Attribute changed:', name, 'Old:', oldValue, 'New:', newValue); // Log attribute change
        ...
        if (name === 'selected-index' && oldValue !== null && oldValue !== newValue) {
            ...
            const fromButtonId = fromButton.getAttribute('aria-controls');
            const toButtonId = toButton.getAttribute('aria-controls');
            this.transition(document.getElementById(fromButtonId), document.getElementById(toButtonId));

            const fromItem = document.querySelector(`[aria-controls-item="${fromButtonId}"]`);
            const toItem = document.querySelector(`[aria-controls-item="${toButtonId}"]`);
            if (fromItem && toItem) {
                this.transition(fromItem, toItem);
            }
        }
    }
...
class CountdownTimer extends HTMLElement {
    ...
    get date() {
        return (this._date =
            this._date ||
            new Date(
                `${this.getAttribute('data-month')}/${this.getAttribute('data-day')}/${this.getAttribute('data-year')} ${this.getAttribute('data-hour')}:${this.getAttribute('data-minute')}:00`
            ));
    }
    ...
    init() {
        ...
        Motion.scroll(
            Motion.animate(
                ...
                {
                    clipPath: ['inset(20% 35% 0% 35% round var(--rounded-block))', 'inset(0% 0%)']
                },
                {easing: 'linear'}
            ),
...
class SplittingBanner extends HTMLElement {
    ...
    init() {
        ...
        Motion.scroll(Motion.animate(wrapper, {opacity: theme.config.mqlSmall ? [1, 0, 0] : [1, 1, 0]}, {easing: 'linear'}), {
            target: tracker,
            offset: Motion.ScrollOffset.Exit
        });
...
class BundleQuantityInput extends HTMLElement {
    constructor() {
        super();
        ...
        this.container.classList.add('quantity-selector');

        // 创建减少按钮
        this.decreaseButton = document.createElement('button');
        this.decreaseButton.innerHTML = '◁'; // 使用符号或图标
        this.decreaseButton.classList.add('quantity-decrease');
        ...
        this.quantityInput.addEventListener('input', this.onInputChange.bind(this));

        // 创建增加按钮
        this.increaseButton = document.createElement('button');
        this.increaseButton.innerHTML = '▷'; // 使用符号或图标
        this.increaseButton.classList.add('quantity-increase');
        this.increaseButton.addEventListener('click', this.increase.bind(this));

        // 组装 HTML 结构
        this.container.appendChild(this.decreaseButton);
        this.container.appendChild(this.quantityInput);
        this.container.appendChild(this.increaseButton);
...
// 注册自定义元素
customElements.define('bundle-quantity-input', BundleQuantityInput);
...
class HeyPopup extends HTMLElement {
    ...
    onInit = () => {
        ...
        theme.pubsub.subscribe(theme.pubsub.PUB_SUB_EVENTS.popupShow, this.onShow.bind(this));
        theme.pubsub.subscribe(theme.pubsub.PUB_SUB_EVENTS.popupHide, this.onHide.bind(this));
    };
}
